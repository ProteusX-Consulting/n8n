...
class WebsiteAnalyzer {
  ...
  async saveResults(filename = null, outputDir = null) {
    if (!filename) {
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const url = new URL(this.analysisData.metadata.url);
      const domain = url.hostname;
      
      // Use current working directory if no output dir specified
      if (!outputDir) {
        outputDir = process.cwd();
      }
      
      // Create website-analysis directory in current location
      const analysisDir = path.join(outputDir, 'website-analysis');
      if (!fs.existsSync(analysisDir)) {
        fs.mkdirSync(analysisDir, { recursive: true });
      }
      
      filename = path.join(analysisDir, `${domain.replace(/\./g, '_')}_analysis_${timestamp}.json`);
    }
...
Examples:
  ...
  wa https://example.com -o ./analysis-results
  website-analyzer https://github.com --output ./reports

Output:
  Creates 'website-analysis' folder with timestamped JSON files containing:
  - Design tokens (colors, fonts, spacing)
  - Performance metrics
  - Complete element data with CSS
  - Component patterns
  - Responsive analysis
  `);
}
...
async function main() {
  ...
  try {
    ...
    console.log(`📁 Output will be saved to: ${outputDir || process.cwd()}/website-analysis/`);
    
    // Analyze both desktop and mobile
    await analyzer.analyzePage(url, { width: 1920, height: 1080 });
    await analyzer.analyzeResponsive(url);
    
    const outputFile = await analyzer.saveResults(null, outputDir);
    console.log(`\n✅ Analysis complete! Results saved to: ${outputFile}`);
    console.log(`📊 Found ${analyzer.analysisData.elements.length} elements with ${analyzer.analysisData.designTokens.colors.length} unique colors`);
